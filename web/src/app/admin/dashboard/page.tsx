...
export default function AdminDashboardPage() {
  ...
  const statCards = [
    { label: 'ผู้ใช้ทั้งหมด', value: stats?.totalUsers ?? 0, icon: <Users size={22} className="text-blue-500" />, bg: 'bg-blue-50' },
    { label: 'ฟาร์มทั้งหมด', value: stats?.totalFarms ?? 0, icon: <Store size={22} className="text-green-500" />, bg: 'bg-green-50' },
    { label: 'คำสั่งซื้อ', value: stats?.totalOrders ?? 0, icon: <ShoppingBag size={22} className="text-orange-500" />, bg: 'bg-orange-50' },
    { label: 'รายได้รวม', value: `฿${Number(stats?.revenue ?? 0).toLocaleString()}`, icon: <Banknote size={22} className="text-purple-500" />, bg: 'bg-purple-50' },
  ]
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-4">
        <div className="flex items-center justify-between mb-4">
          <p className="text-sm font-semibold text-gray-700">ภาพรวมคำสั่งซื้อ</p>
          <div className="flex gap-1">
            {(['day', 'week', 'month', 'year'] as const).map((p) => {
              const label = { day: 'วันนี้', week: 'สัปดาห์นี้', month: 'เดือนนี้', year: 'ปีนี้' }[p]
              return (
                <button key={p} onClick={() => setPeriod(p)}
                  className={`px-3 py-1 rounded-lg text-sm font-medium transition ${period === p ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                  {label}
                </button>
              )
            })}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {statCards.map((s) => (
          <div key={s.label} className={`${s.bg} rounded-xl p-5`}>
            ...
            <p className="text-sm text-gray-700 mt-1 font-medium">{s.label}</p>
          </div>
        ))}
      </div>
    </div>
